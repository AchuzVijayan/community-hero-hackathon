...
// Configure JSON body limits to support base64 image uploads
...
function initDB(): DatabaseSchema {
  ...
  // Pre-populate with high quality default data
  const initialData: DatabaseSchema = {
    ...
    wards: [
      ...
      { id: "ward_4", name: "Vyttila Ward", city: "Kochi", state: "Kerala", health_score: 88, lat: 9.9674, lng: 76.3190 }
    ],
    departments: [
    ...
    issues: [
      ...
      },
      {
        ...
        upvoted_by: ["demo_citizen"],
        is_duplicate: false,
        sla_deadline: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(), // Breached SLA!
    ...
    verifications: [
      { id: "ver_1", issue_id: "issue_1", user_id: "demo_citizen", type: "confirm", created_at: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
      { id: "ver_2", issue_id: "issue_2", user_id: "user_789", type: "confirm", created_at: new Date(Date.now() - 12 * 60 * 60 * 1000).toISOString() }
    ],
    ward_score_history: [
      { id: "h_1", ward_id: "ward_3", score: 41, reason: "Multiple unresolved street and pothole reports", created_at: new Date().toISOString() },
      { id: "h_2", ward_id: "ward_1", score: 92, reason: "Successful resolution of Fort Kochi pipeline leakage within SLA", created_at: new Date().toISOString() }
    ],
    communities: [
      {
        id: "comm_1",
...
// Recalculate health scores for each ward based on open/resolved issues
  ...
  dbData.wards.forEach((ward) => {
    ...
    wardIssues.forEach((issue) => {
      if (issue.status !== 'resolved') {
        ...
        else deduction = 5; // Damaged Footpath / Others

        // Extra severity modifiers
        if (issue.severity >= 8) deduction += 3;
        
        // SLA breach additional penalty
...
const CITY_CONFIGS: Record<string, { lat: number, lng: number, wards: string[] }> = {
  "kochi": {
    lat: 9.9658,
    lng: 76.2421,
...
function getLocalBodyNamesForDistrict(district: string): string[] {
  const dLower = district.trim().toLowerCase();
  
  // A map of districts to their principal local bodies for database generation/filtering
  const DISTRICT_LOCAL_BODIES: Record<string, string[]> = {
...
function ensureCityData(cityRaw: string) {
  ...
  // If this city is not a predefined one, let's cluster it around Kochi/Kerala or its own center if available
  if (!CITY_CONFIGS[city]) {
    // Generate a hash offset so different local bodies don't overlap
    let hash = 0;
    for (let i = 0; i < cityRaw.length; i++) {
      hash = cityRaw.charCodeAt(i) + ((hash << 5) - hash);
    }
    hash = Math.abs(hash);
    const offsetLat = ((hash % 200) - 100) * 0.0008; // +/- 0.08
    const offsetLng = (((hash >> 7) % 200) - 100) * 0.0008; // +/- 0.08
  ...
  // Generate some realistic issues for these new wards!
  ...
  newWards.forEach((ward, wardIdx) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const issueObj = {
        ...
        latitude: ward.lat + template.offsetLat + (i * 0.001),
        longitude: ward.lng + template.offsetLng - (i * 0.001),
        address: `${template.address}, ${ward.name}`,
        upvotes: 5 + (wardIdx * 4) + (i * 3),
        upvoted_by: [],
        is_duplicate: false,
        sla_deadline: new Date(Date.now() + ((isResolved ? -2 : 3) * 24 * 60 * 60 * 1000)).toISOString(),
        created_at: new Date(Date.now() - (5 * 24 * 60 * 60 * 1000)).toISOString(),
        estimated_cost: template.estimated_cost,
        before_image: template.category === "Pothole" ? "https://images.unsplash.com/photo-1515162305285-0293e4767cc2?auto=format&fit=crop&q=80&w=400" :
                     template.category === "Water Leakage" ? "https://images.unsplash.com/photo-1542013936693-8848e5740a9a?auto=format&fit=crop&q=80&w=400" :
                     template.category === "Broken Streetlight" ? "https://images.unsplash.com/photo-1509023464722-18d996393ca8?auto=format&fit=crop&q=80&w=400" :
                     "https://images.unsplash.com/photo-1611284446314-60a58ac0deb9?auto=format&fit=crop&q=80&w=400"
      };

      if (isResolved) {
        (issueObj as any).resolved_at = new Date(Date.now() - (1 * 24 * 60 * 60 * 1000)).toISOString();
        (issueObj as any).after_image = "https://images.unsplash.com/photo-1581094288338-2314dddb7ecc?auto=format&fit=crop&q=80&w=400";
      }

      dbData.issues.push(issueObj);
    }

    // Also add a community group for this ward
...
app.post("/api/auth/login", (req, res) => {
  ...
  // Check password (accept "password123" by default if password is not set on user)
...
app.post("/api/auth/signup", (req, res) => {
  ...
  // Check duplicates
  const duplicate = Object.values(dbData.users).find(u => 
  ...
  // Ensure the dynamically created ward exists
...
// Create Authority Person (Restricted to existing admin/authority only)
...
app.post("/api/communities/:id/leave", (req, res) => {
  ...
  if (comm.members) {
    comm.members = comm.members.filter((mId: string) => mId !== userId);
    saveDB();
  }
...
// Create Issue with AI analysis support
app.post("/api/issues", (req, res) => {
  ...
  // Double check if this is a duplicate within 150m of same category
  const radius = 0.0015; // roughly ~150 meters
  const dup = dbData.issues.find(i => 
    i.category === category &&
    i.status !== 'resolved' &&
    Math.abs(i.latitude - latitude) < radius &&
    Math.abs(i.longitude - longitude) < radius
  );

  const issueId = `issue_${Date.now()}`;
  
  // Base SLA calculation depending on category
  let slaDays = 3;
  ...
  const newIssue = {
    ...
    upvoted_by: [],
    is_duplicate: !!dup,
    parent_issue_id: dup ? dup.id : undefined,
    sla_deadline: new Date(Date.now() + slaDays * 24 * 60 * 60 * 1000).toISOString(),
    created_at: new Date().toISOString(),
...
app.post("/api/issues/:id/upvote", (req, res) => {
  ...
  if (issue.upvotes === undefined) issue.upvotes = issue.upvoted_by.length;
  if (issue.downvotes === undefined) issue.downvotes = issue.downvoted_by.length;

  // If already downvoted, remove the downvote first
...
// Community Verify Endpoint
app.post("/api/issues/:id/verify", (req, res) => {
  const { id } = req.params;
  const { userId, type } = req.body; // type: 'confirm' or 'resolve'
  ...
  // Auto transition to 'verified' if we get 3+ citizen confirmations
...
// Get issue details along with comments
...
// Authority Status Update Endpoint
app.post("/api/issues/:id/status", (req, res) => {
  const { id } = req.params;
  const { status, assigned_to, after_image } = req.body; // status: verified, in_progress, resolved
...
// AI Vision Photo Analysis Endpoint
  ...
  try {
    ...
    const imagePart = {
      inlineData: {
        ...
        data: base64Image.split(",")[1] || base64Image, // Strip out metadata header if present
      },
    };
    ...
    // Fallback if AI call fails or offline keys
    res.json({
...
// AI Text Description Analysis Endpoint
...
// AI Complaint & RTI Letter Generator
...
function new_date_format(isoStr: string) {
  if (!isoStr) return new Date().toLocaleDateString();
...
// ---------------------- Dev Server Setup / Production static server ----------------------
